...
!-----------------------------------------------------------------------
   ...
   use constituents, only: pcnst, pnats, cnst_name, cnst_longname, &
                           dcconnam, sflxnam, tottnam, tendnam, cnst_get_ind
...
PRIVATE
   ...
   integer, parameter :: pflds = 2100           ! max number of fields 
...
!  8 if we have 2 tagged regions 12 for 3 tagged region
   integer, parameter :: tbl_hash_oflow_percent = 40
!
...
!-----------------------------------------------------------------------
      use dycore, only: dycore_is
      use water_tracers, only: trace_water,lwtrcland,wtrc_is_vap,ixwti,ixwtx
      ...
      if (trace_water) then
        ...
        do m = ixwti,ixwtx
          if (wtrc_is_vap(m)) then
            ...
            call add_default('SFU'//trim(cnst_name(m)), 1, ' ')
            call add_default('SFD'//trim(cnst_name(m)), 1, ' ')

            call add_default('PRL'//trim(cnst_name(m)), 1, ' ')
            call add_default('PRC'//trim(cnst_name(m)), 1, ' ')
            call add_default('PSL'//trim(cnst_name(m)), 1, ' ')
            call add_default('PSC'//trim(cnst_name(m)), 1, ' ')
            call add_default('PRT'//trim(cnst_name(m)), 1, ' ')

            call add_default('SHI'//trim(cnst_name(m)), 1, ' ')
            call add_default('SHL'//trim(cnst_name(m)), 1, ' ')
            call add_default('ITK'//trim(cnst_name(m)), 1, ' ')

            if (lwtrcland) then
              call add_default('LWG'//trim(cnst_name(m)), 1, ' ')
              call add_default('LWB'//trim(cnst_name(m)), 1, ' ')
              call add_default('LWR'//trim(cnst_name(m)), 1, ' ')
              call add_default('LWD'//trim(cnst_name(m)), 1, ' ')
            end if
...
!
      ...
      if (trace_water) then
        call add_default ('QFLXUP  ', 1, ' ')
        call add_default ('QFLXDN  ', 1, ' ')
      end if
...
#ifdef TAGGING
      use water_tracers, only: trace_water, lwtrcland, ixwti, ixwtx, wtrc_is_vap,tagmask
#else
      use water_tracers, only: trace_water, lwtrcland, ixwti, ixwtx, wtrc_is_vap
#endif
...
!
      ...
      integer tagmaskid              ! ice-fraction id
      integer tsocnid
     
      integer wtqpertid(pcnst+pnats)	! water tracers
      integer wtsicthkid(pcnst+pnats)	! water tracers
      integer wtsnowhiceid(pcnst+pnats)	! water tracers
      integer wtlwgid(pcnst+pnats)	! water tracers
      integer wtlwbid(pcnst+pnats)	! water tracers
...
#endif
#ifdef TAGGING           
         call addvar (id,'TAGMASK ',nf_double,3,dimen3,tagmaskid)
#endif
...
!
! Water tracers: add surface exchange variables
! (match these with inidat)
!
         if (trace_water) then
           do m = ixwti,ixwtx
             if (wtrc_is_vap(m)) then
               call addvar (id,'QPT'//trim(cnst_name(m)),nf_double,3,dimen3,wtqpertid(m))
#if ( ! defined COUP_CSM )
               call addvar (id,'SHI'//trim(cnst_name(m)),nf_double,3,dimen3,wtsnowhiceid(m))
               call addvar (id,'ITK'//trim(cnst_name(m)),nf_double,3,dimen3,wtsicthkid(m))
#endif
               if (lwtrcland) then
                 call addvar (id,'LWG'//trim(cnst_name(m)),nf_double,3,dimen3,wtlwgid(m))
                 call addvar (id,'LWB'//trim(cnst_name(m)),nf_double,3,dimen3,wtlwbid(m))
               end if
             end if
           end do
         end if
...
!
! Water tracers: dump surface quantities
!
      if (trace_water) then
        do m = ixwti, ixwtx
          if (wtrc_is_vap(m)) then
            ...
            call gather_chunk_to_field (1,1,1,plon,qpert_tmp,arr2d)
            if (masterproc) then
               call wrap_put_vara_realx (id, wtqpertid(m), start2d, count2d, arr2d)
            end if
            qpert_tmp(:,:) = wtsicthk(:,m,:)
            call gather_chunk_to_field (1,1,1,plon,qpert_tmp,arr2d)
            if (masterproc) then
               call wrap_put_vara_realx (id, wtsicthkid(m), start2d, count2d, arr2d)
            end if
            qpert_tmp(:,:) = wtsnowhice(:,m,:)
            call gather_chunk_to_field (1,1,1,plon,qpert_tmp,arr2d)
            if (masterproc) then
               call wrap_put_vara_realx (id, wtsnowhiceid(m), start2d, count2d, arr2d)
            end if
            qpert_tmp(:,:) = wtlwg(:,m,:)
            call gather_chunk_to_field (1,1,1,plon,qpert_tmp,arr2d)
            if (masterproc) then
               call wrap_put_vara_realx (id, wtlwgid(m), start2d, count2d, arr2d)
            end if
            qpert_tmp(:,:) = wtlwb(:,m,:)
            call gather_chunk_to_field (1,1,1,plon,qpert_tmp,arr2d)
            if (masterproc) then
               call wrap_put_vara_realx (id, wtlwbid(m), start2d, count2d, arr2d)
            end if
          end if
        end do
      end if
#ifdef TAGGING           
      call gather_chunk_to_field(1,1,1,plon,tagmask,arr2d)
      if (masterproc) then
         call wrap_put_vara_realx (id, tagmaskid, start2d, count2d, arr2d)
      end if
#endif
!
...
!
         call addfld (tottnam(m) , 'kg/kg/s ',pver, 'A',trim(cnst_name(m))//' tendency due to (3d) advection',dyn_decomp)
         call addfld (tendnam(m), 'kg/kg/s ',pver, 'A',trim(cnst_name(m))//' tendency' ,dyn_decomp)
      end do
      ...
      if (trace_water) then
        ...
        do m = ixwti,ixwtx
         if (wtrc_is_vap(m)) then
           ...
           call addfld ('SFU'//trim(cnst_name(m)),'kg/m2/s ',1, 'A',trim(cnst_name(m))//' surface flux to atmosphere',phys_decomp)
           call addfld ('SFD'//trim(cnst_name(m)),'kg/m2/s ',1, 'A',trim(cnst_name(m))//' surface flux from atmosphere',phys_decomp)

           call addfld ('PRL'//trim(cnst_name(m)),'m/s     ',1, 'A',trim(cnst_name(m))//' tracer large scale precipitation',phys_decomp)
           call addfld ('PRC'//trim(cnst_name(m)),'m/s     ',1, 'A',trim(cnst_name(m))//' tracer convective scale precipitation',phys_decomp)
           call addfld ('PSL'//trim(cnst_name(m)),'m/s     ',1, 'A',trim(cnst_name(m))//' tracer large scale snow',phys_decomp)
           call addfld ('PSC'//trim(cnst_name(m)),'m/s     ',1, 'A',trim(cnst_name(m))//' tracer large scale snow',phys_decomp)
           call addfld ('PRT'//trim(cnst_name(m)),'m/s     ',1, 'A',trim(cnst_name(m))//' tracer total precipitation',phys_decomp)
           call addfld ('PRX'//trim(cnst_name(m)),'m/s     ',1, 'X',trim(cnst_name(m))//' tracer maximum precipitation',phys_decomp)

           call addfld ('SHI'//trim(cnst_name(m)),'m       ',1, 'A',trim(cnst_name(m))//' tracer water equivalent snow depth',phys_decomp)
           call addfld ('SHL'//trim(cnst_name(m)),'m       ',1, 'A',trim(cnst_name(m))//' tracer water equivalent snow depth',phys_decomp)
           call addfld ('ITK'//trim(cnst_name(m)),'m       ',1, 'A',trim(cnst_name(m))//' tracer sea ice thickness',phys_decomp)

           call addfld ('LWG'//trim(cnst_name(m)),'kg/kg   ',1, 'A',trim(cnst_name(m))//' tracer land ground composition',phys_decomp)
           call addfld ('LWB'//trim(cnst_name(m)),'kg/kg   ',1, 'A',trim(cnst_name(m))//' tracer land bulk composition',phys_decomp)
           call addfld ('LWR'//trim(cnst_name(m)),'kg/m2/s ',1, 'A',trim(cnst_name(m))//' tracer land runoff',phys_decomp)
           call addfld ('LWD'//trim(cnst_name(m)),'kg/m2/s ',1, 'A',trim(cnst_name(m))//' tracer land drainage',phys_decomp)
         end if
        end do
      end if
!
      ...
      call addfld ('QFLXUP  ','kg/m2/s ',1,    'A','Surface water flux to atmosphere',phys_decomp)
      call addfld ('QFLXDN  ','kg/m2/s ',1,    'A','Surface water flux from atmosphere',phys_decomp)
...
#ifdef TAGGING
      call addfld ('TAGMASK ','fraction',1,'I','TAGMASK Tagging mask',phys_decomp)
#endif
!
