...
!-----------------------------------------------------------------------
!  ##################################################################
!   @40:    use filenames, only: bndtag
!   @701:   namelist /camexp/ bndtag
!   @1475: call mpibcast ( bndtag, len(bndtag), mpichar, 0, mpicom )
!  ##################################################################
!-----------------------------------------------------------------------
...
!
! trace_water = .F.    If true, turn on nwater tracers (use with wisotope)
! wisotope = .F.       If true, define water tracers to be water isotopes
!
...
! not be supported.  
!       
  namelist /camexp/ ctitle  ,ncdata, bndtvs, bndtag, bndtvo, bndtvg , &
...
#endif
      write(6,*)'Time-variant boundary dataset (tag mask) is: ', trim(bndtag)
...
! f-v dynamics specific
   ...
   call mpibcast (bndtag  ,len(bndtag) ,mpichar,0,mpicom)
