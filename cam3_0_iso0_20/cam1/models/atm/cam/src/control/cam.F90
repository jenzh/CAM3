...
program cam
    ...
    !-----------------------------NOTICE------------------------------------
    !
    !            Community Atmospheric Model, $Name: cam3_0_brnchT_release01 $ (CAM)
    !
    !
  ...
  old_fpe_flags = for_set_fpe(new_fpe_flags)
   ...
   call spmdinit ()
    ...
    if (masterproc) then
        ...
        write(6,*)'$Name: cam3_0_brnchT_release01 $ '
    ...
    !
     !datetime：取得系统当前时间Ymd His格式
    call datetime(cdate,ctime)
    !打印当前时间以及模式使用的动力核心模型
    !EUL欧拉动力学 SLD有限体积动态 LR半拉格朗日动力
    !默认EUL
    if (masterproc) then
    ...
    !
    !设置模式运行态相关参数，比如哪些功能的开关变量。
    !water_isotopes模块中的wisotope参数只是用来命名空间里
