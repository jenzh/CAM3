...
!
      ...
      allocate ( arr3d(plon,plev,plat) )
      allocate ( qwtrc_tmp(plon,plev,plat,pcnst+pnats) ) 
!
      if (masterproc) then
!
! Get dimension IDs and lengths 
      ...
      do m=1,pcnst+pnats
         ...
         if (masterproc) then
            ...
            if (cnst_read_iv(m) .and. &
               ...
               else if (wtrc_implements_cnst(cnst_name(m))) then
                  call wtrc_init_cnst(cnst_name(m), qwtrc_tmp, arr3d)
                  write(6,*) '          ', cnst_name(m), ' initialized by "wtrc_init_cnst"'
...
!
! Save inputs Q, CLDLIQ, CLDICE to local array for water tracer
! initialization
!
            qwtrc_tmp(:,:,:,m) = arr3d(:,:,:)
!
...
!-----------------------------------------------------------------------
      ...
      use spmd_dyn, only: npes, compute_gsfactors
#endif
      use water_tracers, only: trace_water, ixwti, ixwtx, wtrc_is_vap, iwspec
      use water_isotopes, only: wiso_get_rsic
!-----------------------------------------------------------------------
      implicit none
!------------------------------Commons----------------------------------
#include <comqfl.h>
...
!
      ...
      real(r8), pointer, dimension(:,:,:,:) :: qcwat, lcwat
!
      real(r8) :: Rsic		   ! isotope ratio of ice pack
!
#if ( defined SPMD )
      integer :: numperlat         ! number of values per latitude band
      integer :: numsend(0:npes-1) ! number of items to be sent
...
!
! For water tracers, set the initial isotopic concentration of the ice
! Also, set any initial sno to the same ratio - this is not a very good
! solution, but easier to code than insisting that wt{sicthk,snowhice}
! are on the input tape. These ARE on the restarts though!
!
      if (trace_water) then
         do c = begchunk,endchunk
           wtsicthk  (:pcols,1,c) = sicthk(:pcols,c)
           wtsnowhice(:pcols,1,c) = snowhice(:pcols,c)
         end do
!
         do m = ixwti, ixwtx
            if (wtrc_is_vap(m)) then
              Rsic =  wiso_get_rsic(iwspec(m))
              do c = begchunk,endchunk
                wtsicthk  (:pcols,m,c) = Rsic*sicthk(:pcols,c)
                wtsnowhice(:pcols,m,c) = Rsic*snowhice(:pcols,c)
              end do
            end if
         end do
      end if
...
!-----------------------------------------------------------------------
      ...
      use phys_grid
      use buffer
      use water_tracers, only: trace_water, wtrc_init_qpert
...
!
! Water tracers: fill qpert array with a sensible guess for te first step
!
        if (trace_water) then
          call wtrc_init_qpert(qpert(:,:,j))
        end if
!
      end do
