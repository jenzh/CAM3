...
#endif
      use water_tracers, only: trace_water, wtrc_implements_cnst, wtrc_init_cnst, wtrc_init_qpert
...
!
      ...
      if ( istat /= 0 ) call endrun
      allocate ( qwtrc_tmp(plon,plev,plat,pcnst+pnats), stat=istat )
      if ( istat /= 0 ) call endrun
...
! Initialize instead of input
               ...
               if (cldcond_implements_cnst(cnst_name(m))) then
                  ...
                  call wtrc_init_cnst(cnst_name(m), qwtrc_tmp, arrxzy)
...
! Save input Q to local array for water tracer initialization
            qwtrc_tmp(:,:,:,m) = arrxzy(:,:,:)
...
! Save inputs CLDLIQ to local array for water tracer initialization
            qwtrc_tmp(:,:,:,m) = arrxzy(:,:,:)
...
! Save inputs CLDICE to local array for water tracer initialization
            qwtrc_tmp(:,:,:,m) = arrxzy(:,:,:)
         end if
...
!$omp parallel do private(lat)
            ...
            enddo
!
! Save all tracers to temp. array
            qwtrc_tmp(:,:,:,m) = arrxzy(:,:,:)
...
! (could also read this from initial file...)
!
        if (trace_water) then
          call wtrc_init_qpert(qpert(:,:,i))
        end if
