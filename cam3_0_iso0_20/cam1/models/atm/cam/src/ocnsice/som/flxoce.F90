...
!
! Modified for extra output for water tracers 
!  David Noone <dcn@colorado.edu> - Thu Jul  1 20:33:21 MDT 2004
! 
!-----------------------------------------------------------------------
...
!------------------------------Arguments--------------------------------
  ...
  real(r8), intent(out) :: rbot(pcols)    ! Density at bottom model level
  real(r8), intent(out) :: ustar_sv(pcols) ! aved version of ustar
  real(r8), intent(out) :: re_sv(pcols)  ! aved version of re
  real(r8), intent(out) :: ssq_sv(pcols) ! aved version of ssq
...
!
! Save diagnostics for constituent fluxes (dcn)
!
     ustar_sv(i) = ustar
     re_sv(i)    = re
     ssq_sv(i)   = ssq
