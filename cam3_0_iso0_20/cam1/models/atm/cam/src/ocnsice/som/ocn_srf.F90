...
!
! Extra variables for water tracers
!
   real(r8) rbot(pcols)  ! Density at bottom model level
   real(r8) ustar(pcols) ! saved version of ustar
   real(r8) re(pcols)    ! saved version of re
   real(r8) ssq(pcols)   ! saved version of ssq
!
...
!
! For water tracers, overwrite fluxes with explict calulation
!
      if (trace_water) then
        call wtrc_flxoce(npts   ,indx   ,rbot   ,zbot   ,qbot   , &
                         ubot   ,vbot   ,sstk   ,ustar  ,re     , &
                         ssq    ,cflx )
