...
! Now here so available for water isotopes 
...
! Arguments
    ...
    real(r8), intent(out) :: fxliq(pcols,pverp)   ! fluxes at the interfaces, liquid (positive = down)
    real(r8), intent(out) :: fxice(pcols,pverp)   ! fluxes at the interfaces, ice    (positive = down)
...
!===============================================================================
  subroutine cldcond_zmconv_detrain(dlf, dif, cld, state, ptend)
!
...
! Arguments
    ...
    real(r8), intent(in) :: dlf(pcols,pver)       ! detrained liquid water from ZM
    real(r8), intent(in) :: dif(pcols,pver)       ! detrained ice water from ZM
...
!!$          ptend%q(i,k,ixcldliq) = dlf(i,k) * cld(i,k)
          ...
          ptend%q(i,k,ixcldice) = dif(i,k)
          ptend%s(i,k)          = dif(i,k)*latice  ! hack to get energy budget correct
						   ! this should teally be explicit 
					           ! in the convection	
!!          ptend%s(i,k)          = 0.
    ...
    call outfld('ZMDIF' ,dif  , pcols,state%lchnk)
...
! register history variables
    ...
    call addfld ('ZMDIF   ','kg/kg/s ',pver, 'A','Detrained ice water from ZM convection'       ,phys_decomp)
...
!!$  subroutine cldcond_tend(state, ptend, dt, pbuf)
  subroutine cldcond_tend(state, ptend, dt, &
       tcwato, qcwato, lcwato, precip, snow, icefrac, rhdfda, rhu00, cldn, &
       evapprec, prodprec, evapsnow, prodsnow, cme, snowh, dqlfz)
!
...
!!$    type(pbuf_fld), intent(inout), dimension(pbuf_size_max) :: pbuf  ! physics buffer
    ...
    real(r8), intent(out) :: evapsnow(pcols,pver)          ! local evaporation of snow
    real(r8), intent(out) :: prodsnow(pcols,pver)          ! local production of snow
    real(r8), intent(out) :: cme     (pcols,pver)          ! local condensation - evaporation of cloud water
    real(r8), intent(out) :: dqlfz  (pcols,pver)           ! change in liquid due to repartitioning (freeze)
...
! Repartition ice and liquid
    dqlfz(:ncol,:pver) = state%q(:ncol,:pver,ixcldliq)
    ...
    dqlfz(:ncol,:pver) = state%q(:ncol,:pver,ixcldliq) - dqlfz(:ncol,:pver)
