...
!	zero           
!
! Modifed to allow constuent (qbot now dimension pcnst+pnats) to allow
! water isotopes.
! David Noone <dcn@colorado.edu> - Thu Jul  1 20:12:58 MDT 2004
!
!
!-----------------------------------------------------------------------
module comsrf
...
! Few extras for water tracers
  real(r8), allocatable:: wtprcsnw(:,:,:)    ! cam tot tracer snow precip
  real(r8), allocatable:: wtsicthk(:,:,:)    ! cam tot sea ice thickness
  real(r8), allocatable:: wtsnowhice(:,:,:)  ! cam tot sea ice snow 
  real(r8), allocatable:: wtsnowhland(:,:,:) ! cam tot land snow

  real(r8), allocatable:: wtlwg(:,:,:)       ! ground soilwater land bucket
  real(r8), allocatable:: wtlwb(:,:,:)       ! bulk soilwater land bucket
...
!     real(r8) :: qbot(pcols)         ! bot level specific humidity
     real(r8) :: qbot(pcols,pcnst+pnats)  ! bot level specific humidity
     ...
     real(r8) :: wtprecsc(pcols,pcnst+pnats)       ! tracer precipitation
     real(r8) :: wtprecsl(pcols,pcnst+pnats)       ! tracer precipitation
     real(r8) :: wtprecc(pcols,pcnst+pnats)        !  tracer precipitation
     real(r8) :: wtprecl(pcols,pcnst+pnats)        !  tracer precipitation
...
!-----------------------------------------------------------------------
    ...
    allocate (wtprcsnw   (pcols,pcnst+pnats,begchunk:endchunk))
    allocate (wtsicthk   (pcols,pcnst+pnats,begchunk:endchunk))
    allocate (wtsnowhice (pcols,pcnst+pnats,begchunk:endchunk))
    allocate (wtsnowhland(pcols,pcnst+pnats,begchunk:endchunk))

    allocate (wtlwg(pcols,pcnst+pnats,begchunk:endchunk))
    allocate (wtlwb(pcols,pcnst+pnats,begchunk:endchunk))
