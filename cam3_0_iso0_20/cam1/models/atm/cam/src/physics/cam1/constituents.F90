...
!-----------------------------Arguments---------------------------------
!
    ! constituent name used as variable name in history file output (8 char max)
    character(len=*), intent(in) :: name
    ! value for long_name attribute in netcdf output (128 char max, defaults to name)
    character(len=*), intent(in), optional :: longname
    ! true => read initial values from initial file (default: true)
    logical,          intent(in), optional :: readiv
    ! mixing ratio type (dry, wet)
...
!   set constituent type
    ...
    qmin     (ind) = qminc
    qmincg   (ind) = qminc
    !!!!!!!! 没太明白为什么ind == 1时，qmincg = 0,但是确设置在“qmincg(ind) = qminc”语句后。
    !!!!!!按代码流程，ind == 1时，量为Q(Specific humidity),代码在initindx.F90@47
