...
!-----------------------------------------------------------------------
   ...
   use water_tracers,      only: trace_water, lwtrcland, wtrc_init
   use wtrc_cldcond,       only: wtrc_cldcond_init
   use wtrc_zm_conv,       only: wtrc_zm_convi
   use wtrc_land, 	   only: wtrc_land_init
   use wiso_chemistry,     only: wiso_chem_init
   use wiso_histogram,     only: wiso_hist_init
...
!
   !并没有启动，build_isomodel.csh中设置trace_gas = .false.
...
!start to deal with trace_water

   if (trace_water) then
     call wtrc_init
     call wtrc_cldcond_init
     call wtrc_zm_convi
     call wiso_hist_init
     if (trace_gas) then
       call wiso_chem_init
