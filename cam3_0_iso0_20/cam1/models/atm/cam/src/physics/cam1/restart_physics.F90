...
CONTAINS
   ...
   subroutine write_restart_physics (nrg, nrg2)
      ...
      use water_tracers, only: trace_water, ixwti, ixwtx, wtrc_is_vap
! for nlend and aeres
#include <comctl.h>
!
! Input arguments
...
#endif
!
! Water tracers: read/write restart data (makes restart files non-transferable)
!
      do m = ixwti, ixwtx
        if (wtrc_is_vap(m)) then
          tmpfield(:,:) = wtsicthk(:,m,:)
          call write_field_from_chunk(nrg,1,1,1,tmpfield)
          tmpfield(:,:) = wtsnowhland(:,m,:)
          call write_field_from_chunk(nrg,1,1,1,tmpfield)
#if (! defined COUP_CSM)
          tmpfield(:,:) = wtsnowhice(:,m,:)
          call write_field_from_chunk(nrg,1,1,1,tmpfield)
#endif
          tmpfield(:,:) = wtlwg(:,m,:)
          call write_field_from_chunk(nrg,1,1,1,tmpfield)
...
#endif
!
! Water tracers: read/write restart data (makes restart files
! non-transferable)
!
      do m = ixwti, ixwtx
        if (wtrc_is_vap(m)) then
          ...
          wtsicthk(:,m,:) = tmpfield(:,:)
          call read_chunk_from_field(nrg,1,1,1,tmpfield)
          wtsnowhland(:,m,:) = tmpfield(:,:)
#if (! defined COUP_CSM)
          call read_chunk_from_field(nrg,1,1,1,tmpfield)
          wtsnowhice(:,m,:) = tmpfield(:,:)
#endif
          call read_chunk_from_field(nrg,1,1,1,tmpfield)
          wtlwg(:,m,:) = tmpfield(:,:)
          call read_chunk_from_field(nrg,1,1,1,tmpfield)
