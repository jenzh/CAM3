...
!
! Modified to allow all constituents to pass for water tracers.
! David Noone <dcn@colorado.edu> - Thu Jul  1 20:15:19 MDT 2004
! 
!-----------------------------------------------------------------------
...
!   real(r8), intent(in) :: qm1(pcols)           ! Bottom level specific humidity
   real(r8), intent(in) :: qm1(pcols,pcnst+pnats) ! Bottom level specific humidity
...
!
   do m = 1, pcnst+pnats
     do i = 1, ncol
       surface_state2d(lchnk)%qbot(i,m) = qm1(i,m)
     end do
   end do

   call wtrc_qchk1('SRFXFER','qbot',ncol,surface_state2d(lchnk)%qbot(:,ixh2oq),surface_state2d(lchnk)%qbot(:,1))
