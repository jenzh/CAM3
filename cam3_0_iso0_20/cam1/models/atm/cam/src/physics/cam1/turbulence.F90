...
!-----------------------------------------------------------------------
   ...
   !
    ...
    if ( .not. no_outfld ) then 
       ...
       call outfld ('QPERT   ',qpert(:,1),pcols,lchnk)
...
!
    ...
    call wtrc_qchk1('TRBINTD','cflx',ncol,cflx(:,ixh2oq),cflx(:,1))
    call wtrc_qchk1('TRBINTD','kqfs',ncol,kqfs(:,ixh2oq),kqfs(:,1))
!
...
! 
!-----------------------------------------------------------------------
    use water_tracers, only: trace_water, wtrc_is_vap, ixh2oq, wtrc_qchk1
...
!drb initialize variables for runtime error checking
    ...
    do i=1,ncol
       ...
       if (unstbl(i)) then
          ...
          qpert(i,1)   = max(kqfs(i,1)*fak/wm(i),0._r8)
       else
          tpert(i)   = max(khfs(i)*fak/ustar(i),0._r8)
          qpert(i,1)   = max(kqfs(i,1)*fak/ustar(i),0._r8)
       end if
    end do
!
   if (trace_water) then
     do m = 2, pcnst+pnats
       if (wtrc_is_vap(m)) then
         do i = 1, ncol
           if (unstbl(i)) then
             qpert(i,m)   = max(kqfs(i,m)*fak/wm(i),0._r8)

           else
             qpert(i,m)   = max(kqfs(i,m)*fak/ustar(i),0._r8)
           end if

         end do
       end if
     end do
     call wtrc_qchk1('AUSTAU','kqfs',ncol,kqfs(:,ixh2oq),kqfs(:,1))
     call wtrc_qchk1('AUSTAU','qpert',ncol,qpert(:,ixh2oq),qpert(:,1),1.e-12)
   end if
!
