...
!-----------------------------------------------------------------------
    ...
    use water_tracers, only: wtrc_qchk1, wtrc_qchk2,ixh2oq		! debug diags
...
!   array used to calculate tend wrt dry pressure
    ...
    real(r8) qpert_trash(pcols,pcnst+pnats) ! convective humidity and constituent excess (dcn mod)
...
! All variables are modified by vertical diffusion
    ...
    call wtrc_qchk1('VDINTR_in','cflx',ncol,cflx(:,ixh2oq),cflx(:,1))
    call wtrc_qchk2('VDINTR_in','q'   ,ncol,state%q(:,:,ixh2oq),state%q(:,:,1))
...
! Call the real vertical diffusion code.
    call vdiff(                                                            &
     ...
     call wtrc_qchk1('VDIFF','qpert',ncol,qpert(:,ixh2oq),qpert(:,1),1.e-12)
...
!-----------------------------------------------------------------------
    ...
    use water_tracers, only: wtrc_qchk1, ixh2oq
!------------------------------Arguments--------------------------------
    ...
    real(r8), intent(out) :: tpert(pcols)          ! convective temperature excess
    real(r8), intent(out) :: qpert(pcols,pcnst+pnats)          ! convective humidity excess (dcn mod)
...
! Get diffusivities and c-g terms from turbulence module
    call trbintr(lchnk   ,ncol    ,                            &
     ...
     call wtrc_qchk1('VDIFFxtrb','qpert',ncol,qpert(:,ixh2oq),qpert(:,1),1.e-12)
...
! update decomposition in molecular diffusion range
     ...
     call wtrc_qchk1('VDIFF_end','qpert',ncol,qpert(:,ixh2oq),qpert(:,1),1.e-12)
