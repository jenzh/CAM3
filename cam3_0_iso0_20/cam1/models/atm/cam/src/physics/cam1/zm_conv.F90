...
!
   ...
   real(r8), public :: ke                     ! Tunable evaporation efficiency
   real(r8), public :: c0
...
!-----------------------------------------------------------------------
   ...
   use water_tracers, only: trace_water, lwtrczmlin
...
!
   ...
   real(r8), intent(out) :: dif(pcols,pver)    ! scattrd version of the detraining cld h2o tend
...
! transports can be done in outside of conv_cam.
   ...
   real(r8), intent(out) :: cu(pcols,pver)   ! updraft condensation (scattered)
   real(r8), intent(out) :: rr(pcols,pver)   ! updraft rain production rate (scattered)
   real(r8), intent(out) :: evp(pcols,pver)  ! downdraft evaporation (scattered)

   real(r8), intent(out) :: tu(pcols,pver)  ! updraft temperature (scattered)
   real(r8), intent(out) :: td(pcols,pver)  ! downdraft temperature (scattered)


   real(r8), intent(out) :: rliq(pcols) ! reserved liquid (not yet in cldliq) for energy integrals
   real(r8), intent(out) :: rice(pcols) ! reserved ice (not yet in cldice) for energy integrals
   real(r8), intent(out) :: ficed(pcols,pver)     ! fracition ice detrained
   real(r8), intent(out) :: ficeup(pcols,pver)    ! fracition iice at updrafts interfaces
   ...
   real(r8) rrg(pcols,pver)    ! gather rain production
...
!      real(r8) alpha(pcols,pver)      ! array of vertical differencing used (=1. for upstream).
   ...
   real(r8) qug(pcols,pver)
   real(r8) qdg(pcols,pver)
...
!
   do k = msg + 2,pver
      do i = 1,lengath
!            alpha(i,k) = 0.5
         if (trace_water .and. lwtrczmlin) then
...
!
   do k = msg + 1,pver
      do i = 1,lengath
         ...
         rrg  (i,k) = rrg  (i,k)* (zfg(i,k)-zfg(i,k+1))/dp(i,k)
...
!
   call q1q2_pjr(lchnk   , &
                 ...
                 dlg     ,evpg    ,cug     )
!
! Compute the ice fraction of detraining water using updraft temperature
! as well as fraction of ice at interfaces
! (keep ql as BOTH liquid and ice for now - repartition done in tracer core)
!
   call cldwat_fice(lengath, tug , ficeup, fsnow)
!!   ficeup(:,:) = 0.		! test (as before ice/liq mod)
!!   ficeup(:,:) = 1.		! test (ice extreme...)

   do k = msg,pver-1    ! k midpiunt bounded by k+1(lower), and k(upper)
      ficed(:lengath,k) = 0.5*(ficeup(:lengath,k+1) + ficeup(:lengath,k))
   end do
   ficed(:lengath,pver) = ficeup(:lengath,pver)

   fsnow(:,:) = ficed(:,:)	! expell fice, evap repartitions to true fsnow
!
...
!
         ...
         dlf (ideep(i),k) = (1-ficeup(i,min(k+1,pver)))*dlg(i,k)	
         dif (ideep(i),k) = (  ficeup(i,min(k+1,pver)))*dlg(i,k)
         pflx(ideep(i),k) = pflxg(i,k)
         ql  (ideep(i),k) = qlg  (i,k)
         qu  (ideep(i),k) = qug  (i,k)
         qd  (ideep(i),k) = qdg  (i,k)
         cu  (ideep(i),k) = cug  (i,k)
         rr  (ideep(i),k) = rrg  (i,k)
...
! Compute precip by integrating change in water vapor minus detrained cloud water
   do k = pver,msg + 1,-1
      do i = 1,ncol
         prec(i) = prec(i) - dpp(i,k)* (q(i,k)-qh(i,k,1)) - dpp(i,k)*(dlf(i,k)+dif(i,k))*2*delt
      end do
   end do
...
! Treat rliq as flux out bottom, to be added back later.
   do k = 1, pver
      do i = 1, ncol
         ...
         rice(i) = rice(i) + (           dlf(i,k))*dpp(i,k)/gravit
      end do
   end do
...
!===============================================================================
  subroutine zm_conv_evap(state, ptend, prdprec, cldfrc, deltat, &
...
!------------------------------Arguments--------------------------------
    ...
    real(r8), intent(out  ) :: fevprec(pcols,pver)! fraction of precip flux that evaps
    real(r8), intent(out  ) :: fevsnow(pcols,pver)! fraction flux that is snow
...
!
! Save the fraction of precip flux that is evaporated for water tracers
!
          if (flxprec(i,k) > 0.) then
            fevprec(i,k) = evpprec(i) / (flxprec(i,k)*gravit/state%pdel(i,k))
          end if
          if (flxsntm(i) > 0.) then
            fevsnow(i,k) = evpsnow(i) / (flxsntm(i)*gravit/state%pdel(i,k))
          end if
...
! the documentation has been enhanced to the degree that we are able
!
! Mods for water tracers enable full conservation with SMALL changes to climate.
!    0.a) optionally use linear interpolation for midpoints (s,q,h)
!    0.b)
!    1) downdraft top is same as environment
!    2) downdraft limited to ensure positive definite precip flux
!    3) due to 2), recompute downdraft budget. While this SHOULD return
!    saturated values, it does not... which fixes a whole lot of problems.
!
! (David Noone <dcn@colorado.edu> - Mon Jul 19 13:42:35 MDT 2004)
!
!
!-----------------------------------------------------------------------
...
!
   ...
   real(r8), intent(out) :: td(pcols,pver)       ! normalized downdraft temperature
   real(r8), intent(out) :: tu(pcols,pver)       ! normalized updraft temperature
...
!
   real(r8) qdold
   real(r8) sumpcp(pcols),sumevp(pcols) ! precip and evap integrals
   real(r8) efac(pcols)                ! evaporation factor that drives downdrafts
                                       ! "alpha" in ZM paper and CAM docs.
   integer kk
...
!
   ...
   do k = msg + 2,pver
      do i = 1,il2g
!!         if (trace_water .and. lwtrczmlin) then		! use linear (avoids qu < 0, tu > 12000K)
!!           qsthat(i,k) = 0.5*(qst(i,k-1)+qst(i,k))
!!           hsthat(i,k) = cp*shat(i,k) + rl*qsthat(i,k)
!!           gamhat(i,k) = 0.5*(gamma(i,k-1)+gamma(i,k))
!!         else
...
!!               if (hu(i,k) - hsthat(i,k) < -10000) then
!!                   write(*,*) 'dhu(i,k)< -10000 found:',i,k,hu(i,k) - hsthat(i,k)
!!               end if
      ...
      do i = 1,il2g
         if (( .not. done(i) .and. k > jt(i) .and. k < jb(i)) .and. eps0(i) > 0.) then
            ...
            qu(i,k) = mu(i,k+1)/mu(i,k)*qu(i,k+1) + dz(i,k)/mu(i,k)* (eu(i,k)*q(i,k)- &
                            du(i,k)*qst(i,k))
            if (qu(i,k) < 0.) write(*,*) '(zm_cldprp) QU < 0 (a):',i,k,qu(i,k)
            tu(i,k) = su(i,k) - grav/cp*zf(i,k)
            tuone = su(i,k) - grav/cp*zf(i,k)
            estu = c1*exp((c2* (tuone-tfreez))/ ((tuone-tfreez)+c3))
   ...
   do k = msg + 2,pver
      do i = 1,il2g
         if (k == jb(i) .and. eps0(i) > 0.) then
            qu(i,k) = q(i,mx(i))
            if (qu(i,k) < 0.) write(*,*) '(zm_cldprp) QU < 0 (b):',i,k,qu(i,k)
...
!!            if (hu(i,k) > hsthat(i,k)) then
!!               write(*,*) 'BIGGER:',hu(i,k)-hsthat(i,k)
!!            else
!!               write(*,*) 'smaller:',hu(i,k)-hsthat(i,k)
!!            end if
            if (qu(i,k) < 0.) then		! problem occurs as hu-h* is very negative
               write(*,*) '(zm_cldprp) QU < 0 (c):',i,k,qu(i,k)
!!               write(*,*)'HU:', hu(i,k),hsthat(i,k)
!!               write(*,*)'QSh, dhu:', qsthat(i,k),hu(i,k)-hsthat(i,k)

!!              write(*,*) 'H Column',i
!!              write(*,*) jb(i),jt(i),lel(i)
!!              do kk = 1, pver
!!                 write(*,*) kk,hu(i,kk)-hsthat(i,kk)
!!              end do
!              qu(i,k) = max(qu(i,k), 0.)
              qu(i,k) = max(qu(i,k), 1.e-22)
            end if
...
! For water tracers, use the moisture equation, rather than mse
! (analytically the same, numerically different)
   do k = pver,msg + 2,-1
      do i = 1,il2g
         if (k >= jt(i) .and. k < jb(i) .and. eps0(i) > 0.) then
!!            if (trace_water) then	! use water budget (if qu = max(qu0)
!!              cu(i,k)  = (eu(i,k)*q(i,k) - du(i,k)*qst(i,k)) - &
   ...
   do k = pver,msg + 2,-1
      do i = 1,il2g
         ...
         if (k >= jt(i) .and. k < jb(i) .and. eps0(i) > 0. .and. mu(i,k) >= 0.0) then
            ...
            if (ql(i,k) < 0) then
                write(*,*) 'CLDPRP ql < 0:',i,k,ql(i,k),du(i,k)
                ql(i,k) = max(ql(i,k), 0.0)
            endif
            totpcp(i) = totpcp(i) + dz(i,k)*(cu(i,k)-du(i,k)*ql(i,k+1))
            rprd(i,k) = c0*mu(i,k)*ql(i,k)
            rr(i,k) = c0*mu(i,k)*ql(i,k)
         end if
      end do
   end do
!
   do i = 1,il2g
      if (trace_water) then	! this is just could be changed if enough pflx
        qd(i,jd(i)) = q(i,jd(i))		! top of downdraft from environment
      else
        qd(i,jd(i)) = qds(i,jd(i))
      endif
      sd(i,jd(i)) = (hd(i,jd(i)) - rl*qd(i,jd(i)))/cp
      td(i,jd(i)) = sd(i,jd(i)) - grav/cp*zf(i,jd(i))
   end do
!
   do k = msg + 2,pver
      do i = 1,il2g
         if (k >= jd(i) .and. k < jb(i) .and. eps0(i) > 0.) then
            ...
            td(i,k+1) = sd(i,k+1) - grav/cp*zf(i,k+1)
...
!
! Set an mass flux reduction factor
!
   do i = 1, il2g 
     if (totevp(i) > 0. .and. totpcp(i) > 0.) then
       efac(i) = min(1._r8, totpcp(i)/(totevp(i)+totpcp(i)))
     else
       efac(i) = 0.0
     end if
     if (efac(i) < 1.e-12) efac(i) = 0.	! just to be tidy
   end do
!
! Water tracers - require additional constraint so that no more than the
! local precipitation can be evaporated (pflx always positive)
!
   if (trace_water) then
     sumpcp(1:il2g) = 0.
     sumevp(1:il2g) = 0.
!
     do k = 2, pverp
       do i = 1, il2g
          sumpcp(i) =  sumpcp(i) + dz(i,k-1)*rprd(i,k-1)
          if (totevp(i) > 0. .and. totpcp(i) > 0.) then
            if (sumevp(i) > 0.) then
                efac(i) = min(efac(i), sumpcp(i)/sumevp(i))
            end if
          end if
          sumevp(i) =  sumevp(i) + dz(i,k-1)*evp(i,k-1)
       end do
     end do
!
! Write a diagnostic
!!     do i = 1, il2g
!!       if (efac(i) > 0. .and. efac(i) /= totpcp(i)/(totevp(i)+totpcp(i))) then
!!         write(*,*) 'EFAC revised:',i,efac(i),' was:',totpcp(i)/(totevp(i)+totpcp(i)+1.e-20)
!!       end if
!!     end do
   end if		! trace_water
!
...
!!            evp(i,k) = evp(i,k)*min(1._r8, totpcp(i)/(totevp(i)+totpcp(i)))
            md(i,k)  = md (i,k)*efac(i)
            ed(i,k)  = ed (i,k)*efac(i)
            evp(i,k) = evp(i,k)*efac(i)
         else
            md(i,k) = 0.
            ed(i,k) = 0.
            evp(i,k) = 0.
...
!
! If the mass fluxes have been changes, we may need to redo the
! downdraft bass balance
!
   if (trace_water) then
     do k = 1,pver
        do i = 1,il2g
           qd(i,k) = q(i,k) ! Reset to environment as not saturated if no downdraft
       end do
     end do
!
     small = 1.e-20
     do k = msg+2,pver
       do i = 1,il2g
         if (k>=jd(i) .and. k<jb(i) .and. md(i,jb(i))<0.) then
           qdold = qd(i,k+1)
           mdt = min(md(i,k+1),-small)
           qd(i,k+1) = (md(i,k)*qd(i,k) - dz(i,k)*(evp(i,k)+ed(i,k)*q(i,k))) / mdt
!!           if (abs(qd(i,k+1)-qdold) > 1.e-12) then
!!             write(*,*) 'CLDPRP New mass flux -> QD:',qdold,qd(i,k+1)
!!           end if
         end if       ! k, within downdraft plume
       end do         ! i, columns
     end do           ! k, levels
   end if	      ! trace_water
