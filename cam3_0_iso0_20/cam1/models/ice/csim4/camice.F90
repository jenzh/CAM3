...
!JR Renamed Fhnet to Focn
  ...
  use water_tracers,   only: trace_water, wtrc_is_vap, ixwti, ixwtx, wtrc_qchk1, ixh2oq
...
!---------------------------Local variables-----------------------------
  ...
  real(r8) :: wtsnowfall(pcols,pcnst+pnats,begchunk:endchunk) ! water tracer snowfall
...
! Water tracers: accumulate snow
    if (trace_water) then
       wtsnowfall(:,:,c) = 0.
       wtsnowfall(:,1,c) = snowfall(:,c)
       do m = ixwti, ixwtx
         if (wtrc_is_vap(m)) then
           if (prognostic_icesnow) then
             do i = 1, ncol
               wtsnowfall(i,m,c) = srf_state(c)%wtprecsc(i,m) + srf_state(c)%wtprecsl(i,m)
             end do
           else
             write(*,*) 'SEAICE: no code for non-prognostics snow on sea ice'
           end if
         end if
       end do
    end if
! checks
    call wtrc_qchk1('CAMICE_top','snofl',ncol, wtsnowfall(:,ixh2oq,c), snowfall(:,c))
    call wtrc_qchk1('CAMICE_top','sicth',ncol, wtsicthk(:,ixh2oq,c), sicthk(:,c))
    call wtrc_qchk1('CAMICE_top','snohi',ncol, wtsnowhice(:,ixh2oq,c),snowhice(:,c))
    call wtrc_qchk1('CAMICE_top','qbot ',ncol, srf_state(c)%qbot(:,ixh2oq), srf_state(c)%qbot(:,1))
...
! Sea ice surface fluxes and temperatures
     ...
     call seaice (c, ncol, rtime, aice(1,c), tsice(1,c), &
         	  ...
         	  srfflx(c)%lhf, srfflx(c)%lwup, srfflx(c)%tref, fsns(1,c), evap(1,c), &
                  aiceinit(1,c), &
                      wtsicthk(1,1,c), wtsnowhice(1,1,c), wtsnowfall(1,1,c))
    
    call wtrc_qchk1('CAMICE_ice','sicth',ncol, wtsicthk(:,ixh2oq,c), sicthk(:,c))
    call wtrc_qchk1('CAMICE_ice','snohi',ncol, wtsnowhice(:,ixh2oq,c),snowhice(:,c))
!
