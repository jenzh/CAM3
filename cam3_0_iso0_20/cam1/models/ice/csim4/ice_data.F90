...
!
  ...
  use water_tracers, only: trace_water
#if ( defined SPMD )
...
!
              if (.not. prognostic_icesnow) then
                 if (trace_water) then
                    write(*,*) 'ice_data: tracers must have prognostics ice.'
                    call endrun
                 end if
                 if (.not.icecyc) then
...
! !USES:
  ...
  use water_tracers, only: trace_water, wtrc_is_vap, ixwti,ixwtx, iwspec, &
                           wtrc_qchk1, ixh2oq
...
!---------------------------Local variables-----------------------------
  ...
  real(r8) tsurf
  integer :: lats(pcols)           ! array of latitude indices
  real(r8) Rice			! tracer ratio of ice
!
...
!
! For water tracers - set something realistic for initialce ice
!
     if (trace_water) then   
       wtsicthk(:,1,lchnk)   = sicthk(:,lchnk)
       wtsnowhice(:,1,lchnk) = snowhice(:,lchnk)
       do m = ixwti, ixwtx
         if (wtrc_is_vap(m)) then
           Rice = wiso_get_rsic(iwspec(m))
           do i = 1, ncol
              if (icefrac(i,lchnk) > 0._r8) then
                wtsicthk(i,m,lchnk)   = Rice*sicthk(i,lchnk)
                if ( previcefrac(i,lchnk)==0.) then !newice
                   wtsnowhice(i,m,lchnk) = Rice*snowhice(i,lchnk)
                end if
              else
                wtsicthk(i,m,lchnk)   = 0.0
                wtsnowhice(i,m,lchnk) = 0.0
              end if
           end do
         end if
       end do
...
!
    call wtrc_qchk1('ICE_data','sicthk',ncol, wtsicthk(:,ixh2oq,lchnk),sicthk(:,lchnk))
    call wtrc_qchk1('ICE_data','snohic',ncol, wtsnowhice(:,ixh2oq,lchnk),snowhice(:,lchnk))
!
  end do	! lchnk, chunks
  return
